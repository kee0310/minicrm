      ...
      <table class="min-w-full text-sm">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-4 py-3 text-left font-semibold text-gray-700">Approved Bank</th>
            <th class="px-4 py-3 text-left font-semibold text-gray-700">Applied Amount</th>
            <th class="px-4 py-3 text-left font-semibold text-gray-700">Approved Amount</th>
            <th class="px-4 py-3 text-left font-semibold text-gray-700">Interest Rate</th>
            <th class="px-4 py-3 text-left font-semibold text-gray-700">Lock-in Period</th>
            <th class="px-4 py-3 text-left font-semibold text-gray-700">MRTA / MLTA</th>
            <th class="px-4 py-3 text-left font-semibold text-gray-700">Special Conditions</th>
            <th class="px-4 py-3 text-left font-semibold text-gray-700">Deviation %</th>
        ...
        <tbody class="divide-y divide-gray-200">
          @forelse($approvedSubmissions as $submission)
            @php
              ...
              $analysis = $submission->approvalAnalysis;
              $hasRecord = !is_null($analysis);
              $analysisPayload = [
                'deal_id' => $deal?->id,
                'loan_id' => $submission->loan_id,
                'has_record' => $hasRecord,
                'approved_bank' => $analysis?->approved_bank ?? $submission->bank_name,
                'applied_amount' => $analysis?->applied_amount,
                'approved_amount' => $analysis?->approved_amount,
                'interest_rate' => $analysis?->interest_rate,
                'lock_in_period' => $analysis?->lock_in_period,
                'mrta_mlta' => $analysis?->mrta_mlta,
                'special_conditions' => $analysis?->special_conditions,
                'approval_deviation_percentage' => $analysis?->approval_deviation_percentage,
              ];
            ...
            @endphp
            <tr
              x-show="((('{{ strtolower((string) ($deal->deal_id ?? '')) }}' + ' {{ strtolower((string) ($deal->project_name ?? '')) }}' + ' {{ strtolower((string) ($deal->client?->name ?? '')) }}').includes((searchTerm || '').toLowerCase()))) && ((!bankFilter) || ('{{ $analysis?->approved_bank ?? $submission->bank_name ?? '' }}' === bankFilter))">
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3"><b>{{ $analysis?->approved_bank ?? $submission->bank_name ?? '-' }}</b></td>
              <td class="px-4 py-3">{{ $analysis?->applied_amount ?? '-' }}</td>
              <td class="px-4 py-3">{{ $analysis?->approved_amount ?? '-' }}</td>
              <td class="px-4 py-3">{{ $analysis?->interest_rate ?? '-' }}</td>
              <td class="px-4 py-3">{{ $analysis?->lock_in_period ?? '-' }}</td>
              <td class="px-4 py-3">{{ $analysis?->mrta_mlta ?? '-' }}</td>
              <td class="px-4 py-3">{{ $analysis?->special_conditions ?? '-' }}</td>
              <td class="px-4 py-3">{{ $analysis?->approval_deviation_percentage ?? '-' }}</td>
          ...
          <form method="POST" :action="'{{ url('/loans/approval-analysis') }}/' + (editDeal?.deal_id ?? '')">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div><label class="block text-xs font-medium text-gray-600 mb-1">Approved Amount</label><input
                  ...
                  class="w-full rounded-md border-gray-300" /></div>
              <div><label class="block text-xs font-medium text-gray-600 mb-1">Interest Rate</label><input type="number"
                  step="0.01" name="interest_rate" x-model="editDeal.interest_rate"
                  class="w-full rounded-md border-gray-300" /></div>
              <div><label class="block text-xs font-medium text-gray-600 mb-1">Lock-in Period</label><input type="text"
                  name="lock_in_period" x-model="editDeal.lock_in_period" class="w-full rounded-md border-gray-300" />
              </div>
